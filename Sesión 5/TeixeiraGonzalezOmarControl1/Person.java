...
public class Person
{
    ...
    public static final String STANDARD_NAME = "Omar";      //nombre estándar para la persona
    public static final String STANDARD_SURNAME = "Teixeira";       //apellido estándar para la persona
    public static final int STANDARD_AGE = 18;      //edad estándar para la persona
    ...
    private String dni;     //dni para la persona
    private int AgeChanged;     //edad cambiada para el método changeAge
    ...
    public Person()
    {
        setName(STANDARD_NAME);
        setAge(STANDARD_AGE);
        setSurname(STANDARD_SURNAME);
        setGender(STANDARD_GENDER);
        setDni(STANDARD_DNI);
    }
     ...
     * 
     *@param edad con que se crea el objeto, de tipo int
     *@param nombre con el que se crea el objeto, de tipo String
     *@param apellido con el que se crea el objeto, de tipo String
     *@para genero con el que se crea el objeto, de tipo boolean
     */
    ...
    /**
     * Método que modifica la edad de la persona en base a unos determinados 
     *
     */
    public void changeAge()
    {
        if (getGender()==true){
            AgeChanged = (getAge() + getSurname().length()); 
            if (AgeChanged <=100){
                age = AgeChanged;
            }
        }else if(getGender()==false && getAge()>=18 && AgeChanged>= 20){
            AgeChanged = (getAge()-getName().length());
            if (AgeChanged >= 20){
                age = AgeChanged;
            }
        }
